...
import { Home, BookOpen, BarChart2, Calendar, LogOut, Settings, MessageCircle } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  const navItems = [
    ...
    { icon: BookOpen, label: "Friends Progress", href: "/friends" },
    { icon: MessageCircle, label: "Chat", href: "/chat" },
    { icon: BarChart2, label: "Progress", href: "/analytics" },
    ...
    <div className={cn("hidden md:flex flex-col h-screen py-6 pl-4 pr-4 bg-slate-50 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 md:w-72 sticky top-0", className)}>
      <div className="space-y-6 flex-1">
        ...
        <div className="px-4 py-2 flex items-center gap-3 mb-8">
          ...
          <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white font-grotesk">NeuroLearn</h2>
        </div>
              ...
              return (
                <Link key={index} to={item.href}>
                  <Button
                    variant="ghost"
                    className={cn(
                      "w-full justify-start text-base font-medium h-12 rounded-xl transition-all duration-200 mb-1",
                      isActive
                  ...
                  >
                    <item.icon className={cn("mr-3 h-5 w-5", isActive ? "text-white" : "text-slate-400 group-hover:text-indigo-600")} />
                    {item.label}
                  </Button>
                </Link>
              );
      ...
      <div className="mt-auto px-2">
        ...
        <Button 
            ...
            className="w-full justify-start text-slate-500 dark:text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 h-10 rounded-xl"
        >
